...
impl TypeGenerator for BgpNeighbor {
    fn generate<D: Driver>(d: &mut D) -> Option<Self> {
        ...
        Some(BgpNeighbor {
            ...
            remote_asn: d.produce::<u32>()?.to_string(), // Why is this a string
            af_activate: (0..naf)
                .map(|_| d.produce::<BgpAf>().map(|af| af as i32))
    ...
    fn test_bgp_neighbor() {
        ...
        bolero::check!()
            ...
            .for_each(|bgp_neighbor| {
                ...
                if bgp_neighbor.networks.len() > 0 {
                    some_nets = true;
                }
                if bgp_neighbor.af_activate.len() > 0 {
                    some_afs = true;
                }
            });
        assert!(some_nets);
