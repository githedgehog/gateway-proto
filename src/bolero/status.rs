...
impl TypeGenerator for FrrStatus {
    fn generate<D: Driver>(d: &mut D) -> Option<Self> {
        // Weighted distribution for restarts
...
impl TypeGenerator for InterfaceRuntimeStatus {
    fn generate<D: Driver>(d: &mut D) -> Option<Self> {
        ...
        let produced_ic = d.produce::<InterfaceCounters>(); // Option<InterfaceCounters>
        let counters_pick = choose(d, &[produced_ic, None])?; // Option<InterfaceCounters>
...
impl TypeGenerator for BgpNeighborStatus {
    fn generate<D: Driver>(d: &mut D) -> Option<Self> {
        ...
        // small prefixes map: choose AFs from fixed keys
        let mut prefixes = std::collections::HashMap::new();
        let af_keys = ["IPV4_UNICAST", "IPV6_UNICAST", "L2VPN_EVPN"];
        let naf = d.gen_usize(Bound::Included(&0), Bound::Included(&af_keys.len()))?;
        for k in af_keys.iter().take(naf) {
            prefixes.insert((*k).to_string(), d.produce::<BgpNeighborPrefixes>()?);
        }
...
impl TypeGenerator for VpcPeeringCounters {
    fn generate<D: Driver>(d: &mut D) -> Option<Self> {
        ...
        let active_flows = d.gen_u64(Bound::Included(&0), Bound::Included(&50_000))?;
...
impl TypeGenerator for GetDataplaneStatusResponse {
    fn generate<D: Driver>(d: &mut D) -> Option<Self> {
        ...
        // FRR & dataplane status (optionally None)
    ...
    fn test_get_dataplane_status_response() {
        ...
        bolero::check!()
            ...
            .for_each(|resp| {
                ...
                // NEW: interface_runtime values must be sane if present
                ...
                if let Some(bgp) = &resp.bgp {
                    for (vrf, vrf_status) in &bgp.vrfs {
                        ...
                        for (nbr, st) in &vrf_status.neighbors {
                            ...
                            for (af, pfx) in &st.prefixes {
                                assert!(!af.is_empty());
                                assert!(pfx.received <= pfx.received_pre_policy);
                            }
                        }
                    }
                }

                // NEW: VPC status map (keys are VPC names)
