...
mod test {
    ...
    fn test_peering_as() {
        bolero::check!()
            ...
            .for_each(|peering_as: &PeeringAs| {
                ...
                match rule {
                    config_peering_as::Rule::Cidr(cidr) => {
                        assert!(parse_cidr(&cidr).is_ok());
                    }
                    config_peering_as::Rule::Not(cidr) => {
                        assert!(parse_cidr(&cidr).is_ok());
                    }
                }
            });
        ...
        T: FnOnce(IpAddr, u8) -> (),
    {
        let (ip, mask) = match rule {
            config_peering_as::Rule::Cidr(cidr) => parse_cidr(cidr).unwrap(),
            config_peering_as::Rule::Not(cidr) => parse_cidr(cidr).unwrap(),
        };
        test(ip, mask);
    }
